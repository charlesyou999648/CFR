...
class MultimodalLoss(ElementwiseMetric):
    ...
    #     simm = self.sim_fun(features_1.unsqueeze(1), features_2.unsqueeze(0))
    ...
    #     logit = nn.functional.normalize(simm.exp(), dim=1, p=1).log()
        
    #     pos_loss = torch.sum((pos_mask * logit).sum(1) / pos_mask.sum(1)) / pos_mask.shape[0]
    #     # neg_loss = torch.sum(((1-pos_mask) * logit).sum(1) / (1-pos_mask).sum(1)) / pos_mask.shape[0]

    #     if sep_pos_neg:
    #         pos_loss = (pos_mask * logit).mean(-1)
        ...
        for field_idx, field in enumerate(fields):
            ...
            if field == 'y':
                ...
                if self.language_weight > 0.:
                    ...
                    # loss = self.similarity_loss(y_embeddings, y_embeddings, y_embedding_labels, y_embedding_labels, sep_pos_neg=True, abs=False)
            ...
            if field == 'generic-spurious':
                ...
                if self.domain_weight > 0:
                    ...
                    if self.language_weight > 0.:
                        ...
                        loss = - self.pos_weight * loss_pos + self.neg_weight * loss_neg
                        # loss = self.similarity_loss(field_embeddings, field_embeddings, embedding_labels, embedding_labels, sep_pos_neg=True, abs=False)
