...
class Multimodal(SingleModelAlgorithm):
    ...
    def process_batch(self, batch, dropout=0):
        """
        ...
        """
        # Forward pass
        x, y_true, metadata, idxes, weights = batch
        # print('idxes', idxes)
        # print('weights', weights) # only vision/language spurious 
        # exit()
        
        x = x.to(self.device)
        y_true = y_true.to(self.device)
        g = self.grouper.metadata_to_group(metadata).to(self.device)
        y_pred, features = self.model(x, return_features=True, dropout=dropout)
