...
def get_dataset(dataset, version=None, imagenet_class='baby pacifier', seed=42, bingeval=False, commercial=False, **dataset_kwargs):
    ...
    if dataset == 'waterbirds':
        ...
        from waterbirds_dataset_contra_whole import WaterbirdsDataset
        return WaterbirdsDataset(version=version, **dataset_kwargs)
        # from waterbirds_dataset_contra import WaterbirdsDataset
        # return WaterbirdsDataset(version=version, **dataset_kwargs)
    elif dataset == 'imagenet':
        from imagenet_reweight import ImagenetSpurious
        ...
        return CheXpertDataset(version=version, **dataset_kwargs)
    else:
...
def initialize_clip_model(config, featurize=False):
    ...
    # if config.dropout_before:
    #     featurizer.visual._modules[proj_name] = nn.Sequential(
    #         nn.Dropout(config.dropout_before, inplace=True), 
    #         featurizer.visual._modules[proj_name],
    #     )
